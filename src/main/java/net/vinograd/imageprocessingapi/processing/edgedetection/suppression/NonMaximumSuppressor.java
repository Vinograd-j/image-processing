...
public class NonMaximumSuppressor {
    ...
    public Image suppressNonMaximum() {
        return image.mapToNew((x, y) ->{
            ...
            return neighbourIntensities.isEmpty() || Collections.max(neighbourIntensities) > intensity ? new PixelColor(0,0,0) : bound(intensity);
        });
    }

    private PixelColor bound(int intensity){
        int bounded = Math.max(0, Math.min(intensity, 255));
        return new PixelColor(bounded, bounded, bounded);
    }
